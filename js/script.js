import { animate, createTimeline, eases, createSpring, utils, stagger } from 'https://esm.sh/animejs'

/* ====== NOVO: easings Sine (matemáticos puros) ====== */
const customEases = {
...
function updateParameters(state) {
  ...
  const [mass, stiffness, damping, velocity] = params || [];
  const ease =
    state.name === 'createSpring'
      ? (params ? state.ease({ mass, stiffness, damping, velocity }).ease : state.ease().ease)
      : state.ease;

  updateCurve($mainCurve, mainCurvePoints, ease, state.name === 'createSpring' ? undefined : params);
